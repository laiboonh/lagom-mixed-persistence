...
class GreetingsEventProcessor(readSide: JdbcReadSide)(implicit ec: ExecutionContext)
  ...
  val createTableSql =
    "CREATE TABLE IF NOT EXISTS PUBLIC.greetings (name VARCHAR(64), message VARCHAR(512), PRIMARY KEY(name))"
  ...
  val processGreetingMessageChanged: (Connection, EventStreamElement[GreetingMessageChanged]) => Unit = {
    (connection, eventElement) =>
      JdbcSession.tryWith(
        ...
        // We use "MERGE" here because we want this read-side to keep only the latest message per each name
...
//        connection.prepareStatement("MERGE INTO greetings (name, message) VALUES (?, ?)")
        ...
        connection.prepareStatement("INSERT INTO greetings (name, message)" +
          "VALUES(?, ?) " +
          "ON CONFLICT (name) " +
